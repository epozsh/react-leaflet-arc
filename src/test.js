...
L.Arc = L.Polyline.extend({
    ...
    initialize: function (path, options) {//1
    ...
    _setLatLngs: function (path) {//2
        this._coords = path;
        this._bounds = new L.LatLngBounds();
        this._latlngs = this._convertLatLngs(this._computeLatLngs());
    },

    _computeLatLngs: function () {//3
        const from = L.latLng(this._coords.from);
        const to = L.latLng(this._coords.to);
